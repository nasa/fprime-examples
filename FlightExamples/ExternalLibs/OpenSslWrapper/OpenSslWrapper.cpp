...
ExternalLibs::DecryptStatus OpenSslWrapper ::aesDecryptIn_handler(FwIndexType portNum,
                                                                 U16 sa_id,
                                                                 Fw::Buffer& iv,
                                                                 Fw::Buffer& in_buffer) {
    ExternalLibs::DecryptStatus status = ExternalLibs::DecryptStatus::ERROR;
    ...
    aes_decrypt(in_buffer.getData(), in_buffer.getSize(), &this->m_aesKey[0], iv.getData(), this->m_cipherCtx,
                this->m_cipherMethod);
    status = ExternalLibs::DecryptStatus::OK;

    return status;
}

ExternalLibs::DecryptStatus OpenSslWrapper ::aesEncryptIn_handler(FwIndexType portNum,
                                                                 U16 sa_id,
                                                                 Fw::Buffer& iv,
                                                                 Fw::Buffer& in_buffer) {
    ExternalLibs::DecryptStatus status = ExternalLibs::DecryptStatus::ERROR;
...
void OpenSslWrapper ::DEREGISTER_AES_KEY_cmdHandler(FwOpcodeType opCode,  //!< The opcode
                                                    U32 cmdSeq) {         //!< The command sequence number
    this->m_aesKey = ExternalLibs::AesKeyType(0);
